...
CSteamID Steam::createSteamID(uint32_t steam_id, int account_type){
...
bool Steam::addPromoItems(const PackedInt64Array items){
...
bool Steam::exchangeItems(const PackedInt32Array output_items, const uint32 output_quantity, const uint64_t input_items, const uint32 input_quantity){
	if(SteamInventory() == NULL){
		return false;
	}
	return SteamInventory()->ExchangeItems(&inventory_handle, output_items.ptr(), &output_quantity, 1, (const uint64 *)input_items, &input_quantity, 1);
}

// Grants specific items to the current user, for developers only.
bool Steam::generateItems(const PackedInt32Array items, const uint32 quantity){
	if(SteamInventory() == NULL){
		return false;
	}
	return SteamInventory()->GenerateItems(&inventory_handle, items.ptr(), &quantity, items.size());
}
...
//bool Steam::setPNGIcon64x64(const PackedByteArray& icon){
//	if(SteamMusicRemote() == NULL){
//		return false;
//	}
//	return SteamMusicRemote()->SetPNGIcon_64x64(icon.ptr(), icon.size());
//}
...
//bool Steam::updateCurrentEntryCoverArt(const PackedByteArray& art){
//	if(SteamMusicRemote() == NULL){
//		return false;
//	}
//	return SteamMusicRemote()->UpdateCurrentEntryCoverArt(art.ptr(), art.size());
//}
...
void Steam::sendMessages(int messages, const PackedStringArray& message, uint32 connection, int flags){
	if(SteamNetworkingSockets() != NULL){
		SteamNetworkingMessage_t *networkMessage;
		networkMessage = SteamNetworkingUtils()->AllocateMessage(0);
		networkMessage->m_pData = (void *)message.ptr();
...
bool Steam::fileWrite(const String& file, const PackedByteArray& data, int32_t data_size){
	if(SteamRemoteStorage() == NULL){
		return false;
	}
...
void Steam::fileWriteAsync(const String& file, const PackedByteArray& data, int32_t data_size){
	if(SteamRemoteStorage() != NULL){
...
bool Steam::fileWriteStreamWriteChunk(uint64_t write_handle, PackedByteArray& data, int32_t data_size){
	if(SteamRemoteStorage() == NULL){
		return false;
	}
	return SteamRemoteStorage()->FileWriteStreamWriteChunk((UGCFileWriteStreamHandle_t)write_handle, data.ptr(), data_size);
}
...
int Steam::beginAuthSession(PackedByteArray ticket, int ticket_size, uint64_t steam_id){
	...
	return SteamUser()->BeginAuthSession(ticket.ptr(), ticket_size, authSteamID);
}
...
Dictionary Steam::decompressVoice(const PackedByteArray& voice, uint32 voice_size, uint32 sample_rate){
	...
	if(SteamUser() != NULL){
		...
		int result = SteamUser()->DecompressVoice(voice.ptr(), voice_size, &outputBuffer, sizeof(outputBuffer), &written, sample_rate);
		if(result == 0){
...
void Steam::_file_details_result(FileDetailsResult_t* file_data){
	...
	PackedByteArray fileHash;
	fileHash.resize(20);
	for (int i=0; i<20; i++){
		fileHash.insert(i, file_data->m_FileSHA[i]);
	}
